    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    code = code.Substring(0, 12).Trim();
                    var newPasswordHash = UserManager.PasswordHasher.HashPassword(code);
                    user.PasswordHash = newPasswordHash;
                    var result = await UserManager.UpdateAsync(user);
                    if (result.Succeeded)
                    {
                        MailService.SendMail(user.Email, "Forgot Password",
                            $"<b><p>New Password : {code} <br/> , <i>Please Change Password After First Login</i></p></b>");
        ...
        public ActionResult ContactUs(ContactUs contactUs)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    db.ContactUs.Add(new ContactUs()
                    {
                        Id = 0,
